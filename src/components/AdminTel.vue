  ...
  <div class="wrap">
    ...
    <Row>
      <Col span="4" v-for="item in tableData" :key="item._id" style="padding:10px 10px 0 0">
        <Card style="cursor:pointer;height:100px;padding-top:12px" @click.native="onClickDel(item._id)">
          <p class="item-name">{{item.Name}}</p>
          <p class="item-name">{{item.Tel}}</p>
    ...
    <Modal v-model="addModal" width="360">
      ...
      <div style="text-align:center">
        <Form :label-width="40">
          <FormItem label="姓名">
            <Input type="text" v-model="itemName" placeholder="请输入姓名"></Input>
          </FormItem>
          <FormItem label="电话">
            <Input type="text" v-model="phone" placeholder="请输入手机号码"></Input>
          </FormItem>
        </Form>
      </div>
      <div slot="footer">
...
export default {
  ...
  methods: {
    ...
    addItem () {
      ...
      axios.post(apiUrl + '/tels', {Name: this.itemName, Tel: this.phone})
        .then(response => {
